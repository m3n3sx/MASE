...
class AssetLoader {
    ...
    /**
     * 🚀 ENHANCED: Loads assets for MAS V2 admin pages with error handling
     * Features: Asset validation, dependency management, error recovery
     */
    public function enqueueAdminAssets($hook) {
        try {
            ...
            error_log("MAS V2 AssetLoader: Starting admin asset loading for hook: {$hook}");

            // 🎯 STEP 0: Early performance setup
            $this->setupPerformanceOptimizations();
            
            // 🎨 STEP 1: Load critical CSS with validation
            $this->loadCriticalCSS();
            
            // 🚀 STEP 2: Load core JavaScript with dependency management
            ...
            error_log("MAS V2 AssetLoader: Admin asset loading completed successfully");
            
        } catch (Exception $e) {
            error_log("MAS V2 AssetLoader: Failed to load admin assets: " . $e->getMessage());
            $this->handleAssetLoadingError('admin_assets', $e);
        }
    }
    ...
    public function enqueueGlobalAssets($hook) {
        try {
            ...
            error_log("MAS V2 AssetLoader: Starting global asset loading for hook: {$hook}");
            ...
            error_log("MAS V2 AssetLoader: Global asset loading completed successfully");
            
        } catch (Exception $e) {
            error_log("MAS V2 AssetLoader: Failed to load global assets: " . $e->getMessage());
            $this->handleAssetLoadingError('global_assets', $e);
        }
    }
     ...
     */
    private function handleAssetLoadingError($context, $exception) {
        $error_data = [
            'context' => $context,
            ...
            'hook' => $_GET['page'] ?? 'unknown'
        ];
        
        // Store error for diagnostics
        $errors = get_option('mas_v2_asset_errors', []);
        $errors[] = $error_data;
        update_option('mas_v2_asset_errors', array_slice($errors, -15)); // Keep last 15 errors
        
        // Log error
        error_log("MAS V2 AssetLoader Error [{$context}]: " . $exception->getMessage());
        
        // Try to load fallback assets
        $this->loadFallbackAssets($context);
    }

    /**
     * 🔄 Load fallback assets when main assets fail
     */
    private function loadFallbackAssets($context) {
        try {
            switch ($context) {
                case 'admin_assets':
                    // Load minimal CSS fallback
                    wp_enqueue_style(
                        'woow-fallback',
                    ...
                    // Load minimal JS fallback
                    wp_enqueue_script(
                        'woow-fallback-js',
                        $this->plugin_url . 'assets/js/woow-core.js',
                        ['jquery'],
                        $this->plugin_version,
                        true
                    );
                ...
                case 'global_assets':
                    // Load absolute minimal fallback
                    wp_enqueue_style('dashicons');
                    break;
            }
            
            error_log("MAS V2 AssetLoader: Fallback assets loaded for context: {$context}");
            
        } catch (Exception $e) {
            error_log("MAS V2 AssetLoader: Fallback loading also failed: " . $e->getMessage());
        }
    }

    /**
     * ✅ Validate asset file exists before enqueuing
     */
    private function validateAssetFile($file_path, $asset_type = 'unknown') {
        $full_path = $this->plugin_path . 'assets/' . $file_path;
        
        if (!file_exists($full_path)) {
            error_log("MAS V2 AssetLoader: Missing {$asset_type} file: {$full_path}");
            return false;
        }
        
        if (!is_readable($full_path)) {
            error_log("MAS V2 AssetLoader: Unreadable {$asset_type} file: {$full_path}");
            return false;
        }
    ...
    /**
     * 🔧 Enhanced script enqueuing with validation
     */
    private function enqueueScriptSafely($handle, $src, $deps = [], $version = null, $in_footer = true) {
        // Extract relative path for validation
        $relative_path = str_replace($this->plugin_url . 'assets/', '', $src);
        
        if (!$this->validateAssetFile($relative_path, 'JavaScript')) {
            error_log("MAS V2 AssetLoader: Skipping missing script: {$handle}");
            return false;
        }
        
        wp_enqueue_script($handle, $src, $deps, $version ?: $this->plugin_version, $in_footer);
        return true;
    }

    /**
     * 🎨 Enhanced style enqueuing with validation
     */
    private function enqueueStyleSafely($handle, $src, $deps = [], $version = null, $media = 'all') {
        // Extract relative path for validation
        $relative_path = str_replace($this->plugin_url . 'assets/', '', $src);
        
        if (!$this->validateAssetFile($relative_path, 'CSS')) {
            error_log("MAS V2 AssetLoader: Skipping missing style: {$handle}");
            return false;
        }
        
        wp_enqueue_style($handle, $src, $deps, $version ?: $this->plugin_version, $media);
        return true;
    }

    /**
     * 📊 Get asset loading errors for diagnostics
     */
    public function getAssetErrors() {
        return get_option('mas_v2_asset_errors', []);
    }

    /**
     * 🧹 Clear asset loading errors
     */
    public function clearAssetErrors() {
        delete_option('mas_v2_asset_errors');
        return true;
    }

    /**
     * 🔍 Get asset loading diagnostics
     */
    public function getDiagnostics() {
        $diagnostics = [
            ...
            'is_production' => $this->is_production,
            'performance_budget' => $this->performance_budget,
            'asset_errors' => $this->getAssetErrors(),
            'loading_stats' => $this->getLoadingStats(),
        ...
        $diagnostics['critical_assets'] = [];
        foreach ($critical_assets as $asset) {
            $diagnostics['critical_assets'][$asset] = [
                'exists' => $this->validateAssetFile($asset),
                'path' => $this->plugin_path . 'assets/' . $asset,
                'url' => $this->plugin_url . 'assets/' . $asset
            ];
        }
