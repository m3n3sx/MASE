...
class AjaxSecurityManager {
     ...
     * ENHANCED: Progressive rate limiting with IP-based tracking
    ...
    public function checkRateLimit($user_id, $ip_address, $limit) {
        // Create separate keys for user and IP tracking
        $user_key = self::RATE_LIMIT_PREFIX . 'user_' . $user_id;
        $ip_key = self::RATE_LIMIT_PREFIX . 'ip_' . md5($ip_address);
        $now = time();
        
        // Check user-based rate limit
        $user_requests = get_transient($user_key) ?: [];
        $user_requests = array_filter($user_requests, function($timestamp) use ($now) {
            return $timestamp > ($now - self::RATE_LIMIT_WINDOW);
        });
        
        // Check IP-based rate limit (stricter for anonymous/suspicious activity)
        $ip_requests = get_transient($ip_key) ?: [];
        $ip_requests = array_filter($ip_requests, function($timestamp) use ($now) {
            return $timestamp > ($now - self::RATE_LIMIT_WINDOW);
        });
        
        // Progressive rate limiting: stricter limits for repeated violations
        $violation_history = $this->getViolationHistory($user_id, $ip_address);
        $adjusted_limit = $this->calculateAdjustedRateLimit($limit, $violation_history);
        
        // Check if user limit exceeded
        if (count($user_requests) >= $adjusted_limit) {
            $this->logSecurityViolation('rate_limit_exceeded', [
                'user_id' => $user_id,
                'ip_address' => $ip_address,
                'request_count' => count($user_requests),
                'limit' => $adjusted_limit,
                'original_limit' => $limit,
                'window_seconds' => self::RATE_LIMIT_WINDOW,
                'violation_history_count' => count($violation_history),
                'limit_type' => 'user'
            ]);
        ...
        // Check if IP limit exceeded (global IP protection)
        $ip_limit = $adjusted_limit * 2; // Allow higher limit for IP (multiple users)
        if (count($ip_requests) >= $ip_limit) {
            $this->logSecurityViolation('rate_limit_exceeded', [
                'user_id' => $user_id,
                'ip_address' => $ip_address,
                'request_count' => count($ip_requests),
                'limit' => $ip_limit,
                'window_seconds' => self::RATE_LIMIT_WINDOW,
                'limit_type' => 'ip'
            ]);
            ...
            throw new SecurityException(
                sprintf(
                    __('IP rate limit exceeded. Too many requests from this IP address. Please wait before making more requests.', 'woow-admin-styler'),
                    $ip_limit
                ),
                'rate_limit_exceeded'
            );
        ...
        // Add current request to both histories
        $user_requests[] = $now;
        $ip_requests[] = $now;
        set_transient($user_key, $user_requests, self::RATE_LIMIT_WINDOW);
        set_transient($ip_key, $ip_requests, self::RATE_LIMIT_WINDOW);
    }
    
    /**
     * Get violation history for progressive rate limiting
     * Requirements: 6.4, 6.5
     */
    private function getViolationHistory($user_id, $ip_address) {
        $violations = get_option($this->violation_storage_key, []);
        $recent_threshold = strtotime('-1 hour');
        
        return array_filter($violations, function($v) use ($user_id, $ip_address, $recent_threshold) {
            return strtotime($v['timestamp']) > $recent_threshold &&
                   (($v['context']['user_id'] ?? 0) == $user_id || 
                    ($v['context']['ip_address'] ?? '') === $ip_address);
        });
    }
    
    /**
     * Calculate adjusted rate limit based on violation history
     * Requirements: 6.4, 6.5
     */
    private function calculateAdjustedRateLimit($base_limit, $violation_history) {
        $violation_count = count($violation_history);
        
        if ($violation_count === 0) {
            return $base_limit;
        }
        
        // Reduce limit by 20% for each recent violation, minimum 1 request
        $reduction_factor = 1 - ($violation_count * 0.2);
        $adjusted_limit = max(1, intval($base_limit * $reduction_factor));
