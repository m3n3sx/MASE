...
(function (window, document) {
    ...
    class MicroPanelFactory {
        ...
        /**
         * 🎛️ Build individual control element - Refactored for centralized mappings
         */
        buildControl(option) {
            ...
            switch (option.type) {
                ...
                case 'gradient':
                    control = this.buildGradientPicker(option);
                    break;

                case 'multi-slider':
                    control = this.buildMultiValueSlider(option);
                    break;

                case 'font-picker':
                    control = this.buildFontPicker(option);
                    break;
            ...
            // Set option ID for centralized mapping lookup
            control.setAttribute('data-option-id', option.id);
            
            // Remove hardcoded data-css-var - now using centralized mapping
            // Legacy support: only set if explicitly provided in option config
            if (option.cssVar) {
                control.setAttribute('data-css-var-legacy', option.cssVar);
            }
            
            // Keep unit and body class attributes for now (may be moved to mapping later)
        ...
        /**
         * 🎨 Build gradient picker control
         */
        buildGradientPicker(option) {
            const container = document.createElement('div');
            container.className = 'woow-gradient-picker';
            container.setAttribute('data-option-id', option.id);

            // Gradient type selector
            const typeSelect = document.createElement('select');
            typeSelect.className = 'woow-gradient-type';
            ...
            // Direction/angle input for linear gradients
            const directionInput = document.createElement('input');
            directionInput.type = 'range';
            directionInput.min = '0';
            directionInput.max = '360';
            directionInput.value = '90';
            directionInput.className = 'woow-gradient-direction';

            const directionLabel = document.createElement('span');
            directionLabel.className = 'woow-gradient-direction-label';
            directionLabel.textContent = '90°';

            // Color stops container
            const stopsContainer = document.createElement('div');
            stopsContainer.className = 'woow-gradient-stops';

            // Add initial color stops
            const defaultStops = option.default || [
                { color: '#ff0000', position: 0 },
                { color: '#0000ff', position: 100 }
            ];

            defaultStops.forEach((stop, index) => {
                const stopElement = this.createGradientStop(stop, index);
            ...
            addStopBtn.className = 'woow-add-gradient-stop';
            addStopBtn.textContent = '+ Add Stop';

            // Preview element
            const preview = document.createElement('div');
            preview.className = 'woow-gradient-preview';
            this.updateGradientPreview(preview, typeSelect.value, directionInput.value, defaultStops);

            // Event listeners
            typeSelect.addEventListener('change', () => {
                this.updateGradientPreview(preview, typeSelect.value, directionInput.value, this.getGradientStops(stopsContainer));
                this.triggerGradientChange(container);
            });

            directionInput.addEventListener('input', () => {
                directionLabel.textContent = directionInput.value + '°';
                this.updateGradientPreview(preview, typeSelect.value, directionInput.value, this.getGradientStops(stopsContainer));
                this.triggerGradientChange(container);
            });
            ...
            addStopBtn.addEventListener('click', () => {
                ...
                const stopElement = this.createGradientStop(newStop, stopsContainer.children.length);
                stopsContainer.appendChild(stopElement);
                this.updateGradientPreview(preview, typeSelect.value, directionInput.value, this.getGradientStops(stopsContainer));
                this.triggerGradientChange(container);
            });
         ...
         */
        createGradientStop(stop, index) {
            const stopElement = document.createElement('div');
            stopElement.className = 'woow-gradient-stop';

            const colorInput = document.createElement('input');
            colorInput.type = 'color';
            colorInput.value = stop.color;
            colorInput.className = 'woow-stop-color';
            ...
            positionInput.className = 'woow-stop-position';

            const positionLabel = document.createElement('span');
            positionLabel.className = 'woow-stop-position-label';
            positionLabel.textContent = stop.position + '%';

            const removeBtn = document.createElement('button');
            removeBtn.type = 'button';
            removeBtn.className = 'woow-remove-stop';
            removeBtn.textContent = '×';

            // Event listeners
            [colorInput, positionInput].forEach(input => {
                input.addEventListener('input', () => {
                    if (input === positionInput) {
                        positionLabel.textContent = input.value + '%';
                    }
                    const container = stopElement.closest('.woow-gradient-picker');
                    const preview = container.querySelector('.woow-gradient-preview');
                    const typeSelect = container.querySelector('.woow-gradient-type');
                    const directionInput = container.querySelector('.woow-gradient-direction');
                    const stopsContainer = container.querySelector('.woow-gradient-stops');
            ...
            removeBtn.addEventListener('click', () => {
                if (stopElement.parentElement.children.length > 2) { // Keep at least 2 stops
                    stopElement.remove();
                    const container = stopElement.closest('.woow-gradient-picker');
                    this.triggerGradientChange(container);
                }
         ...
         */
        updateGradientPreview(preview, type, direction, stops) {
            ...
            switch (type) {
                case 'linear':
                    gradient = `linear-gradient(${direction}deg, ${stopStrings.join(', ')})`;
                    break;
                case 'radial':
                    gradient = `radial-gradient(circle, ${stopStrings.join(', ')})`;
                    break;
                case 'conic':
                    gradient = `conic-gradient(from ${direction}deg, ${stopStrings.join(', ')})`;
                    break;
            }
        ...
        /**
         * 📊 Get gradient stops from container
         */
        getGradientStops(container) {
            const stops = [];
            container.querySelectorAll('.woow-gradient-stop').forEach(stopElement => {
                const color = stopElement.querySelector('.woow-stop-color').value;
                const position = parseInt(stopElement.querySelector('.woow-stop-position').value);
        ...
        /**
         * 🔔 Trigger gradient change event
         */
        triggerGradientChange(container) {
        ...
        /**
         * 📏 Build multi-value slider control (for spacing: top, right, bottom, left)
         */
        buildMultiValueSlider(option) {
            const container = document.createElement('div');
            container.className = 'woow-multi-slider';
            container.setAttribute('data-option-id', option.id);

            const values = option.default || { top: 0, right: 0, bottom: 0, left: 0 };
            const min = option.min || 0;
            const max = option.max || 100;
            const unit = option.unit || 'px';

            // Link toggle for synchronized values
            const linkToggle = document.createElement('button');
            linkToggle.type = 'button';
            linkToggle.className = 'woow-multi-slider-link';
            linkToggle.innerHTML = '🔗';
            linkToggle.title = 'Link values';
            let isLinked = false;
            ...
            directions.forEach(direction => {
                ...
                sliderWrapper.className = 'woow-slider-wrapper';

                const label = document.createElement('label');
                label.className = 'woow-slider-label';
                label.textContent = direction.charAt(0).toUpperCase() + direction.slice(1);

                const slider = document.createElement('input');
                slider.type = 'range';
                slider.min = min;
                slider.max = max;
                slider.value = values[direction] || 0;
                slider.className = 'woow-multi-slider-input';
                ...
                // Event listener for individual slider
                slider.addEventListener('input', () => {
                    ...
                    if (isLinked) {
                        ...
                        directions.forEach(dir => {
                            if (dir !== direction) {
                                sliders[dir].value = slider.value;
                                sliders[dir].nextElementSibling.textContent = slider.value + unit;
                            }
                        });
                    }
                    
                    this.triggerMultiSliderChange(container);
                });

                sliders[direction] = slider;
            ...
            linkToggle.addEventListener('click', () => {
                isLinked = !isLinked;
                linkToggle.classList.toggle('active', isLinked);
                linkToggle.innerHTML = isLinked ? '🔗' : '🔓';
                linkToggle.title = isLinked ? 'Unlink values' : 'Link values';
            });

            container.insertBefore(linkToggle, container.firstChild);

            return container;
        }

        /**
         * 🔔 Trigger multi-slider change event
         */
        triggerMultiSliderChange(container) {
            const event = new CustomEvent('input', {
                bubbles: true,
                detail: { type: 'multi-slider' }
            });
            container.dispatchEvent(event);
        }

        /**
         * 🔤 Build font picker control with Google Fonts preview
         */
        buildFontPicker(option) {
            const container = document.createElement('div');
            container.className = 'woow-font-picker';
            container.setAttribute('data-option-id', option.id);

            // Font family selector
            const fontSelect = document.createElement('select');
            fontSelect.className = 'woow-font-select';

            // Popular Google Fonts
            const googleFonts = [
                'Inter', 'Roboto', 'Open Sans', 'Lato', 'Montserrat', 'Oswald',
                'Source Sans Pro', 'Raleway', 'PT Sans', 'Lora', 'Merriweather',
                'Playfair Display', 'Poppins', 'Nunito', 'Ubuntu', 'Crimson Text'
            ];

            // System fonts
            const systemFonts = [
                'Arial', 'Helvetica', 'Times New Roman', 'Georgia', 'Verdana',
                'Courier New', 'Trebuchet MS', 'Arial Black', 'Impact'
            ];
            ...
            // Set default value
            fontSelect.value = option.default || 'Inter';

            // Font weight selector
            const weightSelect = document.createElement('select');
            weightSelect.className = 'woow-font-weight';
            const weights = [
            ...
            sizeSlider.max = '72';
            sizeSlider.value = '16';
            sizeSlider.className = 'woow-font-size';

            const sizeLabel = document.createElement('span');
            sizeLabel.className = 'woow-font-size-label';
            sizeLabel.textContent = '16px';

            // Preview text
            const preview = document.createElement('div');
            preview.className = 'woow-font-preview';
            preview.textContent = option.previewText || 'The quick brown fox jumps over the lazy dog';
            
            // Apply initial font
            this.updateFontPreview(preview, fontSelect.value, weightSelect.value, sizeSlider.value);

            // Event listeners
            fontSelect.addEventListener('change', () => {
                this.loadGoogleFont(fontSelect.value);
                this.updateFontPreview(preview, fontSelect.value, weightSelect.value, sizeSlider.value);
                this.triggerFontChange(container);
            });
            ...
            if (googleFonts.includes(fontSelect.value)) {
                this.loadGoogleFont(fontSelect.value);
            }
         ...
         * 🎨 Update font preview
         */
        updateFontPreview(preview, fontFamily, fontWeight, fontSize) {
         ...
         */
        triggerFontChange(container) {
            const event = new CustomEvent('input', {
                bubbles: true,
                detail: { type: 'font-picker' }
            });
            container.dispatchEvent(event);
        }

        /**
         * 🎨 Extract gradient value from gradient picker
         */
        extractGradientValue(container) {
            const typeSelect = container.querySelector('.woow-gradient-type');
            const directionInput = container.querySelector('.woow-gradient-direction');
            const stopsContainer = container.querySelector('.woow-gradient-stops');
            ...
            const stops = this.getGradientStops(stopsContainer);
        ...
        /**
         * 📏 Extract multi-slider values
         */
        extractMultiSliderValue(container) {
            const sliders = container.querySelectorAll('.woow-multi-slider-input');
            const values = {};
            
            sliders.forEach(slider => {
                const direction = slider.getAttribute('data-direction');
                values[direction] = parseInt(slider.value);
            });
        ...
        /**
         * 🔤 Extract font picker values
         */
        extractFontValue(container) {
            const fontSelect = container.querySelector('.woow-font-select');
            const weightSelect = container.querySelector('.woow-font-weight');
            const sizeSlider = container.querySelector('.woow-font-size');
        ...
        /**
         * 📍 ENHANCED: Position panel intelligently with collision avoidance and smooth transitions
         */
        positionPanel(panel, targetElement, preferredPosition = 'bottom-center') {
            ...
            // Get all existing panels for collision detection
            const existingPanels = this.getExistingPanelPositions();
            ...
            // Check if panel should be sticky
            const isSticky = this.shouldPanelBeSticky(panel, targetElement);
            
            if (isSticky) {
                this.applyStickyPositioning(panel, optimalPosition, targetElement);
            } else {
                this.applyStandardPositioning(panel, optimalPosition);
            }

            // Store position for collision detection
            this.storePanelPosition(panel, optimalPosition, { width: panelWidth, height: panelHeight });

            this.log(`Enhanced panel positioned at ${optimalPosition.left}, ${optimalPosition.top}`, {
                preferredPosition,
                finalPosition: optimalPosition,
                isSticky,
                collisionAvoidance: existingPanels.length > 0
            });
        }

        /**
         * 🎯 Calculate optimal position with intelligent collision avoidance
         */
        calculateOptimalPosition(targetRect, panelSize, viewport, preferredPosition, existingPanels) {
            const positions = this.generatePositionCandidates(targetRect, panelSize, preferredPosition);
            ...
            positions.forEach(position => {
                const score = this.scorePosition(position, panelSize, viewport, existingPanels, targetRect);
                if (score > bestScore) {
                    bestScore = score;
        ...
        /**
         * 📊 Generate position candidates in order of preference
         */
        generatePositionCandidates(targetRect, panelSize, preferredPosition) {
            const gap = 10;
            const positions = [];
            ...
            // Add preferred position first
            if (positionMap[preferredPosition]) {
                positions.push(positionMap[preferredPosition]);
            }
            ...
            // Add corner positions as fallbacks
            positions.push(
                { left: targetRect.right + gap, top: targetRect.bottom + gap }, // bottom-right
                { left: targetRect.left - panelSize.width - gap, top: targetRect.bottom + gap }, // bottom-left
                { left: targetRect.right + gap, top: targetRect.top - panelSize.height - gap }, // top-right
                { left: targetRect.left - panelSize.width - gap, top: targetRect.top - panelSize.height - gap } // top-left
            );
        ...
        /**
         * 🏆 Score position based on multiple criteria
         */
        scorePosition(position, panelSize, viewport, existingPanels, targetRect) {
            let score = 100; // Base score

            // Penalty for viewport boundaries
            if (position.left < 10) score -= 50;
            if (position.left + panelSize.width > viewport.width - 10) score -= 50;
            if (position.top < 10) score -= 50;
            if (position.top + panelSize.height > viewport.height - 10) score -= 50;

            // Penalty for collisions with existing panels
            existingPanels.forEach(existingPanel => {
                if (this.checkCollision(position, panelSize, existingPanel.position, existingPanel.size)) {
                    score -= 100; // Heavy penalty for collisions
                }
            });

            // Bonus for staying close to target
            const distance = Math.sqrt(
                Math.pow(position.left - targetRect.left, 2) + 
                Math.pow(position.top - targetRect.top, 2)
            );
            score -= distance * 0.1; // Small penalty for distance

            // Bonus for being within viewport comfortably
            const margin = 20;
        ...
        checkCollision(pos1, size1, pos2, size2) {
            return !(pos1.left + size1.width < pos2.left || 
                     pos2.left + size2.width < pos1.left || 
                     pos1.top + size1.height < pos2.top || 
                     pos2.top + size2.height < pos1.top);
        }

        /**
         * 📍 Get positions of existing panels
         */
        getExistingPanelPositions() {
            const positions = [];
            this.activePanels.forEach(panel => {
                if (panel.style.position === 'fixed' && panel.style.visibility !== 'hidden') {
         ...
         * 📌 Check if panel should be sticky
         */
        shouldPanelBeSticky(panel, targetElement) {
            // Check if this is a frequently used panel
            const panelId = panel.getAttribute('data-element-id');
            const usageCount = this.getPanelUsageCount(panelId);
            
            // Make panel sticky if used frequently or if target element is always visible
            return usageCount > 5 || this.isElementAlwaysVisible(targetElement);
        }

        /**
         * 📊 Get panel usage count (simplified implementation)
         */
        getPanelUsageCount(panelId) {
            const key = `woow_panel_usage_${panelId}`;
            return parseInt(localStorage.getItem(key) || '0');
        }

        /**
         * 👁️ Check if element is always visible (like admin bar)
         */
        isElementAlwaysVisible(element) {
            const alwaysVisibleSelectors = ['#wpadminbar', '#adminmenuwrap'];
            return alwaysVisibleSelectors.some(selector => element.matches(selector));
        }

        /**
         * 📌 Apply sticky positioning
         */
        applyStickyPositioning(panel, position, targetElement) {
            panel.classList.add('woow-panel-sticky');
            
            // Adjust position to be relative to target element
            const targetRect = targetElement.getBoundingClientRect();
            const relativePosition = {
                left: position.left - targetRect.left,
                top: position.top - targetRect.top
            };

            // Store relative position for scroll updates
            panel._stickyOffset = relativePosition;
            panel._stickyTarget = targetElement;
            ...
            this.setupStickyScrollListener(panel);
        }

        /**
         * 📍 Apply standard positioning
         */
        applyStandardPositioning(panel, position) {
            panel.classList.remove('woow-panel-sticky');
            
            // Apply boundary constraints
            const viewport = { width: window.innerWidth, height: window.innerHeight };
            const panelRect = panel.getBoundingClientRect();
            
            position.left = Math.max(10, Math.min(position.left, viewport.width - panelRect.width - 10));
            position.top = Math.max(10, Math.min(position.top, viewport.height - panelRect.height - 10));
        ...
        /**
         * 📜 Setup scroll listener for sticky panels
         */
        setupStickyScrollListener(panel) {
            if (panel._scrollListener) return; // Already set up

            panel._scrollListener = () => {
                if (!panel._stickyTarget || !panel._stickyOffset) return;

                const targetRect = panel._stickyTarget.getBoundingClientRect();
                const newPosition = {
                    left: targetRect.left + panel._stickyOffset.left,
                    top: targetRect.top + panel._stickyOffset.top
                };
                ...
                newPosition.left = Math.max(10, Math.min(newPosition.left, viewport.width - panelRect.width - 10));
                newPosition.top = Math.max(10, Math.min(newPosition.top, viewport.height - panelRect.height - 10));

                panel.style.left = newPosition.left + 'px';
                panel.style.top = newPosition.top + 'px';
            };

            window.addEventListener('scroll', panel._scrollListener, { passive: true });
            window.addEventListener('resize', panel._scrollListener, { passive: true });
        }

        /**
         * 💾 Store panel position for collision detection
         */
        storePanelPosition(panel, position, size) {
            panel._positionData = { position, size, timestamp: Date.now() };
        }
        ...
        /**
         * 🎛️ Handle control value changes - Refactored for centralized mappings
         */
        async handleControlChange(control, event) {
            ...
            } else if (control.classList.contains('woow-gradient-picker')) {
                value = this.extractGradientValue(control);
            } else if (control.classList.contains('woow-multi-slider')) {
                value = this.extractMultiSliderValue(control);
            } else if (control.classList.contains('woow-font-picker')) {
                value = this.extractFontValue(control);
            }
            ...
            // Get CSS variable from centralized mapping system
            const cssVar = this.getCSSVariableFromMapping(optionId);
            
            // Apply changes immediately for real-time feedback
            let cssApplied = false;
            let classApplied = false;

            // Apply CSS variable using centralized mapping
            if (cssVar) {
                cssApplied = await this.applyCSSVariableViaCentralizedMapping(optionId, value, unit);

                // If centralized application failed, try fallback methods
                if (!cssApplied) {
        ...
        /**
         * 🗺️ Get CSS variable from centralized mapping system
         */
        getCSSVariableFromMapping(optionId) {
            // Try enhanced CSS variable mapper first
            if (window.cssVariableMapperInstance && window.cssVariableMapperInstance.getCSSVariable) {
                const cssVar = window.cssVariableMapperInstance.getCSSVariable(optionId);
                if (cssVar) {
                    this.log(`CSS variable from enhanced mapper: ${optionId} → ${cssVar}`);
                    return cssVar;
                }
            }

            // Try refactored CSS getter
            if (window.refactoredCSSGetter && window.refactoredCSSGetter.getCSSVariable) {
                const cssVar = window.refactoredCSSGetter.getCSSVariable(optionId);
                if (cssVar) {
                    this.log(`CSS variable from refactored getter: ${optionId} → ${cssVar}`);
                    return cssVar;
                }
            }

            // Try global getCSSVariable function
            if (window.getCSSVariable && typeof window.getCSSVariable === 'function') {
                const cssVar = window.getCSSVariable(optionId);
                if (cssVar) {
                    this.log(`CSS variable from global function: ${optionId} → ${cssVar}`);
                    return cssVar;
                }
            }

            // Fallback to legacy attribute-based approach for backward compatibility
            const legacyVar = this.getLegacyCSSVariable(optionId);
            if (legacyVar) {
                this.logWarning(`Using legacy CSS variable for ${optionId}: ${legacyVar}`);
                return legacyVar;
            }
        ...
        /**
         * 🎨 Apply CSS variable via centralized mapping system
         */
        async applyCSSVariableViaCentralizedMapping(optionId, value, unit = '') {
            try {
                // Try enhanced CSS variable mapper first (highest priority)
                if (window.cssVariableMapperInstance && window.cssVariableMapperInstance.applyOption) {
                    const success = await window.cssVariableMapperInstance.applyOption(optionId, value, {
                        immediate: true,
                        skipBatch: false
                    });
                    
                    if (success) {
                        this.log(`✅ Applied via enhanced mapper: ${optionId} = ${value}`);
                        return true;
                    }
                }

                // Try CSS Variables Engine
                if (window.cssVariablesEngine && window.cssVariablesEngine.isInitialized) {
                    const success = await window.cssVariablesEngine.applyVariable(optionId, value);
                    if (success) {
                        this.log(`✅ Applied via CSS Engine: ${optionId} = ${value}`);
                        return true;
                    }
                }

                // Try refactored CSS getter with transformation
                if (window.refactoredCSSGetter) {
                    const cssVar = window.refactoredCSSGetter.getCSSVariable(optionId);
                    const transformedValue = window.refactoredCSSGetter.transformValue(optionId, value);
                    
                    if (cssVar && transformedValue !== null) {
                        // Apply the CSS variable directly
                        document.documentElement.style.setProperty(cssVar, transformedValue);
                        document.body.style.setProperty(cssVar, transformedValue, 'important');
                        
                        this.log(`✅ Applied via refactored getter: ${cssVar} = ${transformedValue}`);
                        return true;
                    }
                }
                ...
                const cssVar = this.getCSSVariableFromMapping(optionId);
                if (cssVar) {
                    return await this.applyCSSVariable(cssVar, value, unit);
                }

                return false;

            } catch (error) {
                this.logError(`Failed to apply CSS variable via centralized mapping for ${optionId}:`, error);
                return false;
            }
        }

        /**
         * 🔄 Get legacy CSS variable (backward compatibility)
         */
        getLegacyCSSVariable(optionId) {
            // Check if there's a control with this option ID that has legacy CSS var
            const control = document.querySelector(`[data-option-id="${optionId}"]`);
            if (control) {
                const legacyVar = control.getAttribute('data-css-var-legacy') || control.getAttribute('data-css-var');
                if (legacyVar) {
                    return legacyVar;
                }
            }

            // Fallback to hardcoded mappings for known options
            ...
            return legacyMappings[optionId] || null;
        }
        ...
        getOptionIdFromCSSVar(cssVar) {
            ...
            if (window.cssVariableMapperInstance && window.cssVariableMapperInstance.getOptionIdFromCSSVar) {
                const optionId = window.cssVariableMapperInstance.getOptionIdFromCSSVar(cssVar);
                if (optionId) return optionId;
            }

            // Reverse mapping from CSS var to option ID (fallback)
            const mappings = {
        ...
        applyRemoteChange(optionId, value) {
            ...
            // Apply CSS changes using centralized mapping
            const unit = targetControl.getAttribute('data-unit') || '';
            const bodyClass = targetControl.getAttribute('data-body-class');

            // Use centralized mapping to get CSS variable
            const cssVar = this.getCSSVariableFromMapping(optionId);
            if (cssVar) {
                await this.applyCSSVariableViaCentralizedMapping(optionId, value, unit);
            }
        ...
        /**
         * 🎨 Recover CSS application - Refactored for centralized mappings
         */
        recoverCSSApplication() {
            this.log('Attempting CSS application recovery...');

            // Try to reapply all active CSS variables using centralized mapping
            this.activePanels.forEach(panel => {
                ...
                controls.forEach(control => {
                    ...
                    if (optionId) {
                        ...
                        this.safeExecute(() => {
                            this.applyCSSVariableViaCentralizedMapping(optionId, value, unit);
                        }, 'css-recovery');
                    }
                });
            });
        ...
        /**
         * ❌ ENHANCED: Close panel with sticky positioning cleanup
         */
        closePanel(panel) {
            const elementId = panel.getAttribute('data-element-id');

            // Cleanup sticky positioning
            this.cleanupStickyPositioning(panel);

            // Update panel usage count
            this.updatePanelUsageCount(elementId);
            ...
            // Remove panel with smooth animation
            ...
            this.log(`Enhanced panel closed: ${elementId}`);
        }

        /**
         * 🧹 Cleanup sticky positioning resources
         */
        cleanupStickyPositioning(panel) {
            // Remove scroll listeners
            if (panel._scrollListener) {
                window.removeEventListener('scroll', panel._scrollListener);
                window.removeEventListener('resize', panel._scrollListener);
                delete panel._scrollListener;
            }

            // Clear sticky references
            delete panel._stickyOffset;
            delete panel._stickyTarget;
            delete panel._positionData;
        ...
        /**
         * 📊 Update panel usage count for sticky behavior
         */
        updatePanelUsageCount(panelId) {
            const key = `woow_panel_usage_${panelId}`;
        ...
        /**
         * 🔍 Validate control mapping - Refactored for centralized mappings
         */
        validateControlMapping(control) {
            ...
            // Check if option ID has a mapping in the centralized system
            const cssVar = this.getCSSVariableFromMapping(optionId);
            
            if (!cssVar) {
                this.logError(`No CSS variable mapping found for option: ${optionId}`, {
                    control,
            ...
            if (!cssVar.startsWith('--')) {
                this.logError(`Invalid CSS variable format for ${optionId}: ${cssVar} (must start with --)`, control);
                return false;
            }

            // Check if mapping system is available
            if (!this.isCentralizedMappingAvailable()) {
                this.logWarning(`Centralized mapping system not available, falling back to legacy validation for ${optionId}`);
                return this.validateLegacyControlMapping(control);
            }

            this.log(`✅ Control mapping validated: ${optionId} → ${cssVar}`);
            return true;
        }

        /**
         * 🔍 Check if centralized mapping system is available
         */
        isCentralizedMappingAvailable() {
            return !!(
                (window.cssVariableMapperInstance && window.cssVariableMapperInstance.getCSSVariable) ||
                (window.refactoredCSSGetter && window.refactoredCSSGetter.getCSSVariable) ||
                (window.getCSSVariable && typeof window.getCSSVariable === 'function') ||
                window.CSS_VARIABLE_MAP
            );
        }

        /**
         * 🔍 Legacy control mapping validation (backward compatibility)
         */
        validateLegacyControlMapping(control) {
            const optionId = control.getAttribute('data-option-id');
            const legacyCssVar = control.getAttribute('data-css-var-legacy') || control.getAttribute('data-css-var');

            if (!legacyCssVar) {
                this.logError(`No legacy CSS variable found for ${optionId}`, control);
                return false;
            }

            return this.validateCSSVariableMapping(legacyCssVar, 'test-value');
        }

        /**
         * 📋 Get available mappings for debugging
         */
        getAvailableMappings() {
            const mappings = [];

            // Try to get mappings from various sources
            if (window.CSS_VARIABLE_MAP) {
                mappings.push(...Object.keys(window.CSS_VARIABLE_MAP));
            }

            if (window.cssVariableMapperInstance && window.cssVariableMapperInstance.getMappingsByCategory) {
                const allMappings = window.cssVariableMapperInstance.getMappingsByCategory();
                mappings.push(...Object.keys(allMappings));
            }

            // Remove duplicates and return first 10 for debugging
            return [...new Set(mappings)].slice(0, 10);
        }

        /**
         * 🔍 Validate control mapping completeness for all controls
         */
        validateAllControlMappings() {
            const results = {
                total: 0,
                valid: 0,
                invalid: 0,
                ...
                controls.forEach(control => {
                    ...
                    try {
                        if (this.validateControlMapping(control)) {
                            ...
                            validationResult.cssVar = this.getCSSVariableFromMapping(optionId);
                            results.valid++;
                        } else {
            ...
            // Check for missing mappings in centralized system
            if (window.CSS_VARIABLE_MAP) {
                const configuredOptions = new Set();
                this.panelConfigs.forEach(config => {
                    config.options.forEach(option => {
                        configuredOptions.add(option.id);
                    });
                });

                const mappedOptions = new Set(Object.keys(window.CSS_VARIABLE_MAP));
                
                configuredOptions.forEach(optionId => {
                    if (!mappedOptions.has(optionId)) {
                        results.missing++;
                        results.details.push({
                            ...
                            valid: false,
                            cssVar: null,
                            error: 'Missing from centralized mapping'
                        });
                    }
                });
            }

            this.log('Control mapping validation completed', results);
            return results;
        }
        ...
        updateDebugCSSVariables() {
            ...
            // Get all CSS variables from active controls using centralized mapping
            const allControls = document.querySelectorAll('[data-option-id]');
