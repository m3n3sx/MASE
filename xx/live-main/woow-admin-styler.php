...
spl_autoload_register(function ($class) {
    ...
    if (file_exists($file)) {
        ...
        if (!class_exists($class, false)) {
            error_log("MAS V2 Autoloader: Class '{$class}' not found after loading file '{$file}'");
            
            // Store error for admin display
            $errors = get_option('mas_v2_autoload_errors', []);
            $errors[] = [
                'class' => $class,
                'file' => $file,
                'message' => 'Class not found after file load',
                'timestamp' => time()
            ];
            update_option('mas_v2_autoload_errors', array_slice($errors, -10)); // Keep last 10 errors
        }
    } else {
        error_log("MAS V2 Autoloader: Service file not found: '{$file}' for class '{$class}'");
...
class ModernAdminStylerV2 {
    ...
    /**
     * 🔍 Service Loading Diagnostics
     * Checks the health of service loading and autoloading
     */
    public function runServiceDiagnostics() {
        $diagnostics = [
            'autoloader_errors' => get_option('mas_v2_autoload_errors', []),
            'required_services' => [],
            'service_files' => [],
            'missing_files' => [],
            'class_loading_test' => []
        ...
        foreach ($required_services as $service) {
            $file_path = MAS_V2_PLUGIN_DIR . 'src/services/' . $service . '.php';
            $class_name = 'ModernAdminStyler\\Services\\' . $service;
            
            $diagnostics['service_files'][$service] = [
            ...
            if (!file_exists($file_path)) {
                $diagnostics['missing_files'][] = $service;
            }
            ...
            try {
                ...
                $diagnostics['class_loading_test'][$service] = class_exists($class_name);
            } catch (Exception $e) {
                $diagnostics['class_loading_test'][$service] = 'ERROR: ' . $e->getMessage();
            }
        }
    ...
    /**
     * 🚨 Display Service Loading Errors in Admin
     */
    public function showServiceLoadingErrors() {
        $errors = get_option('mas_v2_autoload_errors', []);
        if (!empty($errors)) {
            add_action('admin_notices', function() use ($errors) {
                echo '<div class="notice notice-error is-dismissible">';
                echo '<h3>WOOW Admin Styler - Service Loading Issues</h3>';
                echo '<p>The following service loading errors were detected:</p>';
                echo '<ul>';
                foreach (array_slice($errors, -5) as $error) { // Show last 5 errors
                    echo '<li><strong>' . esc_html($error['class']) . '</strong>: ' . esc_html($error['message']);
                    echo ' <em>(' . date('Y-m-d H:i:s', $error['timestamp']) . ')</em></li>';
                }
                echo '</ul>';
                echo '<p><a href="' . admin_url('admin.php?page=mas-v2-diagnostics') . '">View Full Diagnostics</a></p>';
                echo '</div>';
            });
        }
    }
    ...
    /**
     * Initialize WordPress compatibility manager
     */
    private function initWordPressCompatibility() {
        try {
            require_once MAS_V2_PLUGIN_DIR . 'src/services/WordPressCompatibilityManager.php';
            $this->compatibility_manager = new \ModernAdminStyler\Services\WordPressCompatibilityManager();
        } catch (Exception $e) {
            error_log('WOOW Admin Styler: Failed to initialize WordPress compatibility: ' . $e->getMessage());
            
            // Add admin notice for compatibility initialization failure
            add_action('admin_notices', function() use ($e) {
                echo '<div class="notice notice-error is-dismissible">';
                echo '<p><strong>WOOW Admin Styler:</strong> WordPress compatibility initialization failed. ';
    ...
    private function init() {
        // Initialize WordPress compatibility first
        $this->initWordPressCompatibility();
        ...
        // 🚀 ENHANCED AJAX SYSTEM: Centralized endpoint management with error handling
        // Note: AJAX endpoints are automatically registered by UnifiedAjaxManager in initServices()
        
        // 🚀 ENTERPRISE FEATURE: Dynamic UI Configuration REST API
        add_action('rest_api_init', [$this, 'registerRestRoutes']);
        
        // Integration system hooks
        add_action('admin_enqueue_scripts', [$this, 'enqueueIntegrationAssets'], 5); // Early priority
        
        // Initialize service diagnostics and error reporting
        $this->showServiceLoadingErrors();
        add_action('admin_menu', [$this, 'addDiagnosticsPage']);
    }
    
    /**
     * 🔧 Add Diagnostics Page to Admin Menu
     */
    public function addDiagnosticsPage() {
        add_submenu_page(
            'mas-v2-admin',
            'WOOW Diagnostics',
            'Diagnostics',
    ...
    public function renderDiagnosticsPage() {
        $diagnostics = $this->runServiceDiagnostics();
        
        echo '<div class="wrap">';
        echo '<h1>🔍 WOOW Admin Styler - Service Diagnostics</h1>';
        ...
        $missing_files = count($diagnostics['missing_files']);
        
        echo '<p><strong>Total Services:</strong> ' . $total_services . '</p>';
        echo '<p><strong>Successfully Loaded:</strong> ' . $loaded_services . '</p>';
        echo '<p><strong>Missing Files:</strong> ' . $missing_files . '</p>';
        
        if ($loaded_services === $total_services && $missing_files === 0) {
            echo '<div class="notice notice-success inline"><p>✅ All services are loading correctly!</p></div>';
        } else {
            echo '<div class="notice notice-error inline"><p>❌ Some services have issues that need attention.</p></div>';
        }
        echo '</div>';
        
        // Detailed Service Information
        echo '<div class="card" style="margin: 20px 0; padding: 20px;">';
        echo '<h2>🔧 Detailed Service Information</h2>';
        echo '<table class="wp-list-table widefat fixed striped">';
        echo '<thead><tr>';
        echo '<th>Service</th>';
        echo '<th>File Exists</th>';
        echo '<th>Class Loaded</th>';
        echo '<th>File Path</th>';
        echo '<th>Status</th>';
        echo '</tr></thead>';
        echo '<tbody>';
        
        foreach ($diagnostics['service_files'] as $service => $info) {
            $status_icon = '❌';
            $status_text = 'Error';
            
            if ($info['file_exists'] && $diagnostics['class_loading_test'][$service] === true) {
                $status_icon = '✅';
                $status_text = 'OK';
            } elseif ($info['file_exists'] && $diagnostics['class_loading_test'][$service] !== true) {
                $status_icon = '⚠️';
                $status_text = 'File exists but class not loaded';
            }
            
            echo '<tr>';
            echo '<td><strong>' . esc_html($service) . '</strong></td>';
            echo '<td>' . ($info['file_exists'] ? '✅ Yes' : '❌ No') . '</td>';
            echo '<td>' . ($diagnostics['class_loading_test'][$service] === true ? '✅ Yes' : '❌ No') . '</td>';
            echo '<td><code>' . esc_html($info['file_path']) . '</code></td>';
        ...
        // Autoloader Errors
        if (!empty($diagnostics['autoloader_errors'])) {
            echo '<div class="card" style="margin: 20px 0; padding: 20px;">';
            echo '<h2>🚨 Recent Autoloader Errors</h2>';
            echo '<table class="wp-list-table widefat fixed striped">';
            echo '<thead><tr>';
            ...
            foreach (array_reverse($diagnostics['autoloader_errors']) as $error) {
                echo '<tr>';
                echo '<td>' . date('Y-m-d H:i:s', $error['timestamp']) . '</td>';
                echo '<td><code>' . esc_html($error['class']) . '</code></td>';
                echo '<td>' . esc_html($error['message']) . '</td>';
                echo '<td><code>' . esc_html($error['file']) . '</code></td>';
                echo '</tr>';
            }
            
            echo '</tbody></table>';
            
            echo '<p><button type="button" class="button" onclick="clearAutoloaderErrors()">Clear Error Log</button></p>';
            echo '<script>
                function clearAutoloaderErrors() {
                    if (confirm("Are you sure you want to clear the autoloader error log?")) {
                        fetch(ajaxurl, {
                            method: "POST",
                            headers: {"Content-Type": "application/x-www-form-urlencoded"},
                            body: "action=mas_clear_autoloader_errors&nonce=' . wp_create_nonce('mas_clear_errors') . '"
        ...
        echo '<tr><th>Plugin Version</th><td>' . MAS_V2_VERSION . '</td></tr>';
        echo '<tr><th>Plugin Directory</th><td><code>' . MAS_V2_PLUGIN_DIR . '</code></td></tr>';
        echo '<tr><th>Plugin URL</th><td><code>' . MAS_V2_PLUGIN_URL . '</code></td></tr>';
        echo '<tr><th>Memory Usage</th><td>' . size_format(memory_get_usage(true)) . '</td></tr>';
    ...
    /**
     * 🔍 Render Error Dashboard Page
     */
    public function renderErrorDashboard() {
        // Include the error dashboard view
        include_once MAS_V2_PLUGIN_DIR . 'src/views/error-dashboard.php';
    }
    
    /**
     * ⚡ Render Performance Dashboard Page
     */
    public function renderPerformanceDashboard() {
        // Include the performance dashboard view
        include_once MAS_V2_PLUGIN_DIR . 'src/views/performance-dashboard.php';
    }
    ...
    /**
     * Enqueue integration system assets
    ...
    public function enqueueIntegrationAssets() {
        ...
        // Enqueue CSS Variable Map (centralized mapping) - Load first
        wp_enqueue_script(
            'woow-css-variable-map',
            MAS_V2_PLUGIN_URL . 'assets/js/css-variable-map.js',
            [], // No dependencies - loaded first
            MAS_V2_VERSION,
            false // Load in head for early availability
        );
        
        // Enqueue CSS Variables Restorer
        wp_enqueue_script(
            'woow-css-variables-restorer',
            MAS_V2_PLUGIN_URL . 'assets/js/css-variables-restorer.js',
            ['woow-css-variable-map'], // Depends on variable map
            MAS_V2_VERSION,
            false // Load in head for early restoration
        );
        
        // Enqueue CSS Variables Initialization System
        wp_enqueue_script(
            'woow-css-variables-init',
            MAS_V2_PLUGIN_URL . 'assets/js/css-variables-initialization-system.js',
            ['woow-css-variables-restorer'], // Depends on restorer
            MAS_V2_VERSION,
            false // Load in head for early initialization
        );
        
        // Enqueue WOOW Admin Integration System (NEW)
        wp_enqueue_script(
            'woow-admin-integration',
            MAS_V2_PLUGIN_URL . 'assets/js/woow-admin-integration-system.js',
            ['woow-css-variables-init'], // Depends on initialization system
            MAS_V2_VERSION,
            true // Load in footer after DOM is ready
        );
        
        // Enqueue CSS Variables Integration System (Legacy compatibility)
        wp_enqueue_script(
            'woow-css-variables-integration',
            MAS_V2_PLUGIN_URL . 'assets/js/css-variables-integration-system.js',
            ['woow-admin-integration'], // Depends on new integration system
            MAS_V2_VERSION,
            true
        );
        
        // Localize script with WordPress integration data
        wp_localize_script('woow-css-variables-integration', 'woowAdminStyler', [
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('woow_css_variables'),
        ...
        // Add inline CSS for integration system
        $integration_css = "
            /* CSS Variables Integration System Styles */
            .woow-integration-loading {
                opacity: 0.7;
                transition: opacity 0.3s ease;
            }
            
            .woow-integration-complete {
                opacity: 1;
                transition: opacity 0.3s ease;
            }
            
            /* Migration indicator */
            .woow-migration-active::before {
                content: '🔄 Migrating...';
                position: fixed;
                top: 32px;
                right: 20px;
                background: var(--woow-accent-primary, #0073aa);
                color: white;
                padding: 8px 12px;
                border-radius: 4px;
                font-size: 12px;
        ...
        wp_add_inline_style('woow-admin-main', $integration_css);
        
        // Localize script with integration system data
        wp_localize_script('woow-admin-integration', 'woowIntegrationConfig', [
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('woow_css_variables'),
            'pluginUrl' => MAS_V2_PLUGIN_URL,
            'version' => MAS_V2_VERSION,
    ...
    /**
     * AJAX endpoint for loading CSS variables from database
     * Requirements: 2.1, 2.2, 2.3
     */
    public function ajaxLoadCSSVariables() {
        // Verify nonce for security
        if (!wp_verify_nonce($_POST['nonce'] ?? '', 'woow_css_variables')) {
            wp_die(json_encode([
                'success' => false,
                'data' => 'Invalid nonce'
            ]));
        ...
        try {
            ...
            // Initialize CSS Variable Manager if not already done
            if (!$this->css_variable_manager) {
                $this->css_variable_manager = new \ModernAdminStyler\Services\CSSVariableManager($this->settings_manager);
            }
            
            // Generate CSS variables from settings
            $css_variables = $this->generateCSSVariablesArray($settings);
            
            // Log successful load
            error_log(sprintf(
                'WOOW CSS Variables: Successfully loaded %d variables for user %d',
                count($css_variables),
                get_current_user_id()
            ));
            ...
            wp_die(json_encode([
                ...
                'count' => count($css_variables)
            ]));
            
        } catch (Exception $e) {
            error_log('WOOW CSS Variables Load Error: ' . $e->getMessage());
            
            wp_die(json_encode([
                'success' => false,
                'data' => 'Failed to load CSS variables: ' . $e->getMessage()
            ]));
        }
    }
    
    /**
     * AJAX endpoint for saving CSS variables to database
     * Requirements: 2.1, 2.2, 2.3
     */
    public function ajaxSaveCSSVariables() {
        // Verify nonce for security
        if (!wp_verify_nonce($_POST['nonce'] ?? '', 'woow_css_variables')) {
            wp_die(json_encode([
                'success' => false,
        ...
        try {
            ...
            if (!is_array($variables)) {
                throw new Exception('Invalid variables data format');
            }
            
            // Convert CSS variables back to settings format
            $settings = $this->convertCSSVariablesToSettings($variables);
            ...
            // Save to database
            $saved = update_option('mas_v2_settings', $updated_settings);
            ...
            // Log successful save
            error_log(sprintf(
                'WOOW CSS Variables: Successfully saved %d variables for user %d',
                count($variables),
                get_current_user_id()
            ));
            ...
            wp_die(json_encode([
                'success' => true,
                'data' => 'CSS variables saved successfully',
                'saved_count' => count($variables),
                'timestamp' => current_time('mysql')
            ]));
            
        } catch (Exception $e) {
            error_log('WOOW CSS Variables Save Error: ' . $e->getMessage());
            
            wp_die(json_encode([
                'success' => false,
                'data' => 'Failed to save CSS variables: ' . $e->getMessage()
            ]));
        }
    }
    ...
    public function ajaxTestConnection() {
        ...
        try {
            ...
            // Test options table access
            $test_option = get_option('mas_v2_settings', null);
            ...
            wp_die(json_encode([
                ...
                'data' => [
                    ...
                    'options_accessible' => $test_option !== null,
                    'timestamp' => current_time('mysql')
                ]
            ]));
            
        } catch (Exception $e) {
            error_log('WOOW Database Connection Test Error: ' . $e->getMessage());
            
            wp_die(json_encode([
                'success' => false,
                'data' => 'Database connection test failed: ' . $e->getMessage()
            ]));
        }
    }
    
    /**
     * Generate CSS variables array from settings
     * Requirements: 5.1, 5.2
     */
    private function generateCSSVariablesArray($settings) {
        $css_variables = [];
        
        // Get component configuration for mapping
        $components_config = $this->getComponentsConfig();
        
        // Extract CSS variable mappings from components config
        foreach ($components_config as $component) {
            if (isset($component['tabs'])) {
                foreach ($component['tabs'] as $tab) {
                    if (isset($tab['options'])) {
                        foreach ($tab['options'] as $option) {
                            if (isset($option['cssVar']) && isset($option['id'])) {
                                $setting_value = $settings[$option['id']] ?? $option['default'] ?? null;
                                
                                if ($setting_value !== null) {
                                    // Convert setting value to CSS value
                                    $css_value = $this->convertSettingToCSSValue($setting_value, $option);
                                    
                                    if ($css_value !== null) {
                                        $css_variables[$option['cssVar']] = $css_value;
                                    }
                                }
                            }
                        }
        ...
        $css_variables = array_merge($default_variables, $css_variables);
     ...
     */
    private function convertSettingToCSSValue($value, $option) {
        ...
        switch ($type) {
            ...
            case 'slider':
                $unit = $option['unit'] ?? '';
                return is_numeric($value) ? $value . $unit : null;
     ...
     */
    private function convertCSSVariablesToSettings($css_variables) {
        $settings = [];
        
        // Get component configuration for reverse mapping
        $components_config = $this->getComponentsConfig();
        
        // Create reverse mapping from CSS variables to setting IDs
        $reverse_mapping = [];
        foreach ($components_config as $component) {
            if (isset($component['tabs'])) {
                foreach ($component['tabs'] as $tab) {
                    if (isset($tab['options'])) {
                        foreach ($tab['options'] as $option) {
                            if (isset($option['cssVar']) && isset($option['id'])) {
                                $reverse_mapping[$option['cssVar']] = $option;
                            }
                        }
                    }
                }
        ...
        // Convert CSS variables to settings
        foreach ($css_variables as $css_var => $css_value) {
            if (isset($reverse_mapping[$css_var])) {
                $option = $reverse_mapping[$css_var];
                $setting_value = $this->convertCSSValueToSetting($css_value, $option);
                
                if ($setting_value !== null) {
                    $settings[$option['id']] = $setting_value;
                }
            }
        }
    ...
    /**
     * Convert CSS value back to setting value
     * Requirements: 5.3, 5.4
     */
    private function convertCSSValueToSetting($css_value, $option) {
        $type = $option['type'] ?? 'string';
        
        switch ($type) {
            case 'color':
                return $css_value; // Colors are stored as-is
                
            case 'slider':
                // Remove unit suffix if present
                $unit = $option['unit'] ?? '';
                if ($unit && str_ends_with($css_value, $unit)) {
                    return floatval(str_replace($unit, '', $css_value));
                }
                return floatval($css_value);
                
            case 'toggle':
            case 'visibility':
                return $css_value === '1' || $css_value === 'true';
     ...
     * Requirements: 5.2, 5.3
     */
    private function sanitizeColorValue($value) {
        // Handle hex colors
        ...
        if (preg_match('/^hsla?\([\d\s,%.]+\)$/i', $value)) {
            return $value;
        }
        
        // Handle named colors (basic validation)
        $named_colors = ['transparent', 'inherit', 'initial', 'unset', 'currentcolor'];
    ...
    /**
     * 🏗️ Enhanced Service Initialization with Error Handling
     * 🎯 NEW CONSOLIDATED ARCHITECTURE: 8 strategic services with dependency injection
     */
    public function initServices() {
        try {
            // Initialize CoreEngine with error handling
            ...
            try {
                ...
                $this->preset_manager = $this->settings_manager; // Consolidated into SettingsManager
                $this->component_adapter = $this->settings_manager; // Integrated into SettingsManager
                error_log('MAS V2: SettingsManager initialized successfully');
            } catch (Exception $e) {
                error_log('MAS V2: Failed to initialize SettingsManager: ' . $e->getMessage());
            ...
            try {
                ...
                $this->metrics_collector = $this->cache_manager; // Consolidated into CacheManager
                $this->analytics_engine = $this->cache_manager; // Consolidated into CacheManager
                $this->memory_optimizer = $this->cache_manager; // Consolidated into CacheManager
                error_log('MAS V2: CacheManager initialized successfully');
            } catch (Exception $e) {
                error_log('MAS V2: Failed to initialize CacheManager: ' . $e->getMessage());
            ...
            try {
                ...
                error_log('MAS V2: SecurityManager initialized successfully');
            } catch (Exception $e) {
                error_log('MAS V2: Failed to initialize SecurityManager: ' . $e->getMessage());
                $this->security_service = null;
            }
            
            // 4. Style Generation
            try {
                $this->css_generator = $coreEngine->getStyleGenerator();
                error_log('MAS V2: StyleGenerator initialized successfully');
            } catch (Exception $e) {
                error_log('MAS V2: Failed to initialize StyleGenerator: ' . $e->getMessage());
                $this->css_generator = null;
            }
            
            // 4.1 CSS Variable Management (NEW)
            try {
                if ($this->settings_manager) {
                    $this->css_variable_manager = new \ModernAdminStyler\Services\CSSVariableManager($this->settings_manager);
                    error_log('MAS V2: CSSVariableManager initialized successfully');
                } else {
                    throw new Exception('SettingsManager required for CSSVariableManager');
                }
            } catch (Exception $e) {
                error_log('MAS V2: Failed to initialize CSSVariableManager: ' . $e->getMessage());
                $this->css_variable_manager = null;
            }
            ...
            try {
                ...
                $this->settings_api = $this->ajax_handler; // Consolidated into CommunicationManager
                $this->rest_api = $this->ajax_handler; // Consolidated into CommunicationManager
                error_log('MAS V2: CommunicationManager initialized successfully');
            } catch (Exception $e) {
                error_log('MAS V2: Failed to initialize CommunicationManager: ' . $e->getMessage());
                $this->ajax_handler = null;
            }
            
            // 5.1 Unified AJAX Management (NEW - Live Edit Mode)
            try {
                if ($this->settings_manager) {
                    $this->unified_ajax_manager = new \ModernAdminStyler\Services\UnifiedAjaxManager($this->settings_manager);
                    error_log('MAS V2: UnifiedAjaxManager initialized successfully');
                } else {
                    throw new Exception('SettingsManager required for UnifiedAjaxManager');
                }
            } catch (Exception $e) {
                error_log('MAS V2: Failed to initialize UnifiedAjaxManager: ' . $e->getMessage());
                $this->unified_ajax_manager = null;
            }
            ...
            try {
                ...
                $this->gutenberg_manager = $this->hooks_manager; // Consolidated into AdminInterface
                error_log('MAS V2: AdminInterface initialized successfully');
            } catch (Exception $e) {
                error_log('MAS V2: Failed to initialize AdminInterface: ' . $e->getMessage());
                $this->hooks_manager = null;
            }
            
            // 7. Asset Loading
            try {
                $this->asset_loader = $coreEngine->getAssetLoader();
                error_log('MAS V2: AssetLoader initialized successfully');
            } catch (Exception $e) {
                error_log('MAS V2: Failed to initialize AssetLoader: ' . $e->getMessage());
                $this->asset_loader = null;
            }
            ...
            try {
                if (file_exists(MAS_V2_PLUGIN_DIR . 'src/services/InputValidator.php')) {
                    ...
                    error_log('MAS V2: InputValidator initialized successfully');
                } else {
                    throw new Exception('InputValidator.php file not found');
                }
            } catch (Exception $e) {
                error_log('MAS V2: Failed to initialize InputValidator: ' . $e->getMessage());
                $this->input_validator = null;
            }
            
            // Count successfully initialized services
            $initialized_services = 0;
            $service_list = [
                'settings_manager', 'cache_manager', 'security_service', 'css_generator',
                'css_variable_manager', 'ajax_handler', 'hooks_manager', 'asset_loader', 'input_validator'
            ];
            ...
            error_log("MAS V2: Service initialization completed - {$initialized_services}/" . count($service_list) . " services loaded successfully");
            
            // Store initialization status for diagnostics
            update_option('mas_v2_service_init_status', [
                'total_services' => count($service_list),
                'initialized_services' => $initialized_services,
                'last_init_time' => time(),
                'init_success' => $initialized_services > 0
            ]);
            
        } catch (Exception $e) {
            error_log('MAS V2: Critical error during service initialization: ' . $e->getMessage());
            
            // Store critical error for admin display
            update_option('mas_v2_critical_init_error', [
    ...
    public function addAdminMenu() {
        ...
        */
        ...
        // Error Dashboard - Task 7: Comprehensive Error Handling
        add_submenu_page(
            'woow-v2-general',
            '🔍 Error Dashboard',
            '🔍 Error Dashboard',
            'manage_options',
            'woow-v2-error-dashboard',
            [$this, 'renderErrorDashboard']
        );
        
        // Performance Dashboard - Task 8: Performance Monitoring
        add_submenu_page(
            'woow-v2-general',
            '⚡ Performance Dashboard',
            '⚡ Performance Dashboard',
            'manage_options',
            'woow-v2-performance-dashboard',
    ...
    public function enqueueAssets($hook) {
        try {
            // Use enhanced AssetLoader with error handling
            if ($this->asset_loader) {
                $this->asset_loader->enqueueAdminAssets($hook);
            } else {
                error_log('MAS V2: AssetLoader not available, loading fallback assets');
                $this->loadFallbackAssets($hook);
            }
            
            // Enhanced localization for WOOW pages
            $woow_pages = [
                ...
                // Add MAS pages for compatibility
            ...
            if (in_array($hook, $woow_pages)) {
                ...
                $this->localizeAdminScripts($hook);
            }
            
        } catch (Exception $e) {
            error_log('MAS V2: Asset enqueuing failed: ' . $e->getMessage());
            $this->loadFallbackAssets($hook);
        }
    }
     ...
     */
    private function localizeAdminScripts($hook) {
        try {
            $localization_data = [
                'ajaxUrl' => admin_url('admin-ajax.php'),
                ...
                'strings' => [
                    'saving' => __('Saving...', 'woow-admin-styler'),
                    'saved' => __('Settings saved successfully!', 'woow-admin-styler'),
                    'error' => __('An error occurred while saving', 'woow-admin-styler'),
                    'confirm_reset' => __('Are you sure you want to reset to default settings?', 'woow-admin-styler'),
                    'resetting' => __('Resetting...', 'woow-admin-styler'),
                    'reset_success' => __('Settings have been reset!', 'woow-admin-styler'),
                    'loading' => __('Loading...', 'woow-admin-styler'),
                    'connection_error' => __('Connection error. Please try again.', 'woow-admin-styler')
                ],
                'capabilities' => [
            ...
            // Try to localize multiple script handles for compatibility
            $script_handles = [
                'woow-v2-admin',
                'woow-core',
            ...
            foreach ($script_handles as $handle) {
                if (wp_script_is($handle, 'enqueued')) {
                    wp_localize_script($handle, 'woowV2Global', $localization_data);
                    error_log("MAS V2: Localized script handle: {$handle}");
                }
            }
            
        } catch (Exception $e) {
            error_log('MAS V2: Script localization failed: ' . $e->getMessage());
        }
    }
    
    /**
     * 🔄 Load Fallback Assets when AssetLoader fails
     */
    private function loadFallbackAssets($hook) {
        try {
            // Load minimal CSS
            wp_enqueue_style(
                'woow-fallback-css',
            ...
            wp_enqueue_script(
                'woow-fallback-js',
                MAS_V2_PLUGIN_URL . 'assets/js/woow-core.js',
                ['jquery'],
                MAS_V2_VERSION,
                true
            );
            ...
            // Basic localization
            wp_localize_script('woow-fallback-js', 'woowV2Global', [
                'ajaxUrl' => admin_url('admin-ajax.php'),
                'nonce' => wp_create_nonce('mas_v2_ajax_nonce'),
                'pluginUrl' => MAS_V2_PLUGIN_URL,
                'fallbackMode' => true
            ...
            error_log('MAS V2: Fallback assets loaded successfully');
            
        } catch (Exception $e) {
            error_log('MAS V2: Fallback asset loading also failed: ' . $e->getMessage());
        }
    }
    
    /**
     * 🌐 Enhanced Global Asset Loading with Error Handling
     */
    public function enqueueGlobalAssets($hook) {
        try {
            // Skip WOOW admin pages (they have their own assets)
            $woow_pages = [
                'toplevel_page_woow-v2-general',
                'woow-admin_page_woow-v2-general',
                'woow-admin_page_woow-v2-admin-bar',
            ...
            if (in_array($hook, $woow_pages)) {
                return; // Skip global assets on WOOW pages
            }
            ...
            if ($this->asset_loader) {
                ...
                error_log('MAS V2: AssetLoader not available for global assets, loading minimal fallback');
                $this->loadMinimalGlobalFallback();
            }
            
            // Add early loading protection if available
            if ($this->asset_loader && method_exists($this->asset_loader, 'addEarlyLoadingProtection')) {
                add_action('admin_head', [$this->asset_loader, 'addEarlyLoadingProtection'], 1);
            }
            
        } catch (Exception $e) {
            error_log('MAS V2: Global asset enqueuing failed: ' . $e->getMessage());
            $this->loadMinimalGlobalFallback();
        }
    }
    
    /**
     * 🔄 Load minimal global fallback assets
     */
    private function loadMinimalGlobalFallback() {
        try {
            // Only load essential global styles
            wp_enqueue_style('dashicons');
            
            // Load minimal theme support if not on login page
            ...
            error_log('MAS V2: Even minimal global fallback failed: ' . $e->getMessage());
        }
    }
    ...
    /**
     * Transform WordPress settings to CSS variables format
     * @param array $settings WordPress option settings
     * @return array CSS variables in --woow-* format
     */
    private function transformSettingsToCSSVariables($settings) {
        $css_variables = [];
        
        // Get components configuration for mapping
        $components_config = $this->getComponentsConfig();
        
        // Create a mapping from setting ID to CSS variable
        $setting_to_css_map = [];
        foreach ($components_config as $component) {
            if (isset($component['tabs'])) {
                foreach ($component['tabs'] as $tab) {
                    if (isset($tab['options'])) {
                        foreach ($tab['options'] as $option) {
                            if (isset($option['cssVar'])) {
                                $setting_to_css_map[$option['id']] = $option['cssVar'];
                            }
                        }
                    }
                }
        ...
        // Transform settings using the mapping
        foreach ($settings as $setting_id => $value) {
            if (isset($setting_to_css_map[$setting_id])) {
                $css_var_name = $setting_to_css_map[$setting_id];
                $css_variables[$css_var_name] = $this->formatCSSVariableValue($value, $setting_id);
            } else {
                // Fallback transformation for unmapped settings
                $css_var_name = $this->transformSettingIdToCSSVariable($setting_id);
                $css_variables[$css_var_name] = $this->formatCSSVariableValue($value, $setting_id);
            }
        }
    ...
    /**
     * Transform setting ID to CSS variable name (fallback method)
     * @param string $setting_id WordPress setting ID
     * @return string CSS variable name
     */
    private function transformSettingIdToCSSVariable($setting_id) {
        // Remove 'woow_' prefix if present
        $css_var = $setting_id;
        if (strpos($css_var, 'woow_') === 0) {
            $css_var = substr($css_var, 5);
        }
    ...
    /**
     * Format CSS variable value based on setting type
     * @param mixed $value Setting value
     * @param string $setting_id Setting ID for context
     * @return string Formatted CSS value
     */
    private function formatCSSVariableValue($value, $setting_id) {
        // Handle boolean values (toggles)
        if (is_bool($value)) {
            return $value ? '1' : '0';
        }

        // Handle numeric values (sliders)
        if (is_numeric($value)) {
            // Add 'px' unit for dimension-related settings
            if (strpos($setting_id, 'width') !== false || 
                strpos($setting_id, 'height') !== false || 
                strpos($setting_id, 'padding') !== false || 
                strpos($setting_id, 'margin') !== false || 
    ...
    /**
     * Get default CSS variables for fallback
    ...
    private function getDefaultCSSVariables() {
        return [
            ...
            '--woow-surface-menu-width' => '160px',
            '--woow-surface-content' => '#ffffff',
            '--woow-content-max-width' => '1400px',
            '--woow-content-padding' => '20px',
            '--woow-surface-footer' => '#f1f1f1',
            '--woow-surface-footer-text' => '#666666',
            '--woow-surface-postbox' => '#ffffff',
            '--woow-surface-postbox-border' => '#e0e0e0',
            '--woow-surface-postbox-text' => '#333333',
            '--woow-radius-postbox' => '8px',
            '--woow-surface-postbox-padding' => '15px',
            '--woow-surface-postbox-margin' => '10px',
            '--woow-text-primary' => '#1d2327',
            '--woow-text-secondary' => '#646970',
            '--woow-accent-primary' => '#2271b1',
            '--woow-accent-secondary' => '#72aee6',
    ...
    /**
     * 🧹 AJAX Clear Autoloader Errors
     */
    public function ajaxClearAutoloaderErrors() {
        if (!wp_verify_nonce($_POST['nonce'] ?? '', 'mas_clear_errors') || !current_user_can('manage_options')) {
            wp_send_json_error(['message' => 'Security check failed']);
            return;
        }
        
        delete_option('mas_v2_autoload_errors');
        wp_send_json_success(['message' => 'Autoloader errors cleared successfully']);
    }
