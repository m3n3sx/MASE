...
(function($) {
    ...
    class SettingsPageEnhancer {
        ...
        setupThemeSwitcher() {
            // Theme toggle button
            $('#mas-v2-theme-toggle').on('click', (e) => {
                ...
                setTimeout(() => $btn.removeClass('rotating'), 500);
        ...
        setupConditionalFields() {
            ...
            console.log('[MAS Settings] Found conditional fields:', $conditionalFields.length);
            ...
            $conditionalFields.each(function() {
                ...
                console.log('[MAS Settings] Conditional field:', showWhen, '=', showValue);
                
                const checkVisibility = () => {
                    const $trigger = $(`#${showWhen}, [name="${showWhen}"]`);
                    console.log('[MAS Settings] Trigger found:', $trigger.length, 'is checkbox:', $trigger.is(':checkbox'), 'checked:', $trigger.is(':checked'));
                    ...
                    if ($trigger.is(':checkbox')) {
                        shouldShow = $trigger.is(':checked') && (showValue == '1' || showValue == 'true');
                    } else {
                        shouldShow = $trigger.val() == showValue;
                    }
                    
                    console.log('[MAS Settings] Should show:', shouldShow);
                ...
                // Listen for changes
                $(`#${showWhen}, [name="${showWhen}"]`).on('change input', checkVisibility);
            });
        }
