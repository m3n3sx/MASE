...
class ModernAdminStylerV2 {
    ...
    private function init() {
        ...
        // Initialize Unified CSS Manager
        ...
        // else: Unified CSS Manager handles everything (loaded in init_unified_css_manager)
    ...
    private function init_unified_css_manager() {
        ...
        if (is_admin()) {
            $css_admin_file = MAS_V2_PLUGIN_DIR . 'includes/admin/class-mas-css-manager-admin.php';
            if (file_exists($css_admin_file)) {
                require_once $css_admin_file;
                MAS_CSS_Manager_Admin::getInstance();
            }
        }
    }
    ...
    public function enqueueAssets($hook) {
        ...
        // Add inline menu CSS to ensure consistency
        ...
        // 3. Unified Live Preview v2.0 - All 3 Phases Complete
        wp_enqueue_script(
            'mas-v2-unified-live-preview',
            MAS_V2_PLUGIN_URL . 'assets/js/unified-live-preview-v2.js',
        ...
        wp_enqueue_style(
            'mas-v2-plugin-ui',
            MAS_V2_PLUGIN_URL . 'assets/css/plugin-ui.css',
            ['mas-v2-global'],
            MAS_V2_VERSION
        );
        
        // Inline critical CSS
        $inline_css = '.mas-v2-card{background:#fff;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,0.08);padding:24px;margin-bottom:20px}.mas-v2-card h2{margin:0 0 20px 0;font-size:20px;font-weight:600;color:#1d2327;border-bottom:2px solid #0073aa;padding-bottom:12px}.mas-v2-field-group{margin-bottom:24px}.mas-v2-field-label{display:block;font-weight:600;color:#1d2327;margin-bottom:8px;font-size:14px}.mas-v2-input{width:100%;max-width:400px;padding:8px 12px;border:1px solid #8c8f94;border-radius:4px}';
        wp_add_inline_style('mas-v2-global', $inline_css);
        
        // Disable old conflicting systems
        wp_add_inline_script('jquery', 'window.MAS_DISABLE_OLD_PREVIEW = true;', 'before');
    ...
    public function enqueueGlobalAssets($hook) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('MAS V2: enqueueGlobalAssets() is deprecated. Use MAS_Unified_CSS_Manager instead.');
        }
        ...
        // Only load if not already loaded by enqueueAssets
        if (!wp_style_is('mas-v2-global', 'enqueued')) {
            wp_enqueue_style(
                'mas-v2-global',
                MAS_V2_PLUGIN_URL . 'assets/css/admin-modern.css',
                ['mas-v2-menu-cooperative'],
        ...
        // TEMPORARY: Advanced Effects CSS disabled to fix menu
        // wp_enqueue_style(
        ...
        // Minimal menu fix - only submenu background
        wp_enqueue_style(
            'mas-v2-menu-fix-minimal',
            MAS_V2_PLUGIN_URL . 'assets/css/menu-fix-minimal.css',
            array('mas-v2-global'),
            MAS_V2_VERSION
        );
    ...
    public function ajaxGetPreviewCSS() {
        ...
        try {
            ...
            // Merge all changed settings from POST data
            foreach ($_POST as $key => $value) {
                ...
                } elseif ($value === 'true' || $value === 'false' || $value === true || $value === false || $value === '1' || $value === '0') {
                    $settings[$setting_key] = ($value === 'true' || $value === true || $value === '1');
                } else {
    ...
    public function outputCustomStyles() {
        ...
        // Generate CSS - ALWAYS generate menu CSS
        $css = '';
        $css .= $this->generateCSSVariables($settings);
        $css .= $this->generateMenuCSS($settings);
        $css .= $this->generateAdminBarCSS($settings);
        ...
        // ALWAYS output CSS - even if empty
        echo "<style id='mas-v2-dynamic-styles'>\n";
        echo $css;
        // Fix submenu transparency
        echo "\n/* Submenu background fix */\n";
        echo "#adminmenu .wp-submenu { background: #2c3338 !important; }\n";
        echo "#adminmenu .wp-submenu a { color: #fff !important; }\n";
        echo "#adminmenu .wp-submenu a:hover { background: #32373c !important; }\n";
        echo "@media (prefers-color-scheme: dark) {\n";
        echo "    body.wp-admin #adminmenu .wp-submenu { background: #2c3338 !important; }\n";
        echo "    body.wp-admin #adminmenu .wp-submenu a { color: #fff !important; }\n";
        echo "}\n";
        echo "#wpadminbar .ab-submenu { background: #32373c !important; }\n";
        echo "\n</style>\n";
        ...
        // Emergency inline CSS for plugin UI
        echo "<style id='mas-v2-emergency-ui'>\n";
        echo ".mas-v2-card{background:#fff;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,0.08);padding:24px;margin-bottom:20px}";
        echo ".mas-v2-card h2{margin:0 0 20px 0;font-size:20px;font-weight:600;color:#1d2327;border-bottom:2px solid #0073aa;padding-bottom:12px}";
        echo ".mas-v2-field-group{margin-bottom:24px}";
        echo ".mas-v2-field-label{display:block;font-weight:600;color:#1d2327;margin-bottom:8px;font-size:14px}";
        echo ".mas-v2-input,.mas-v2-select{width:100%;max-width:400px;padding:8px 12px;border:1px solid #8c8f94;border-radius:4px;font-size:14px}";
        echo ".mas-v2-button{padding:10px 20px;border:none;border-radius:4px;font-size:14px;font-weight:600;cursor:pointer}";
        echo ".mas-v2-button-primary{background:#0073aa;color:#fff}";
        echo "\n</style>\n";
    ...
    private function generateCSSVariables($settings) {
        ...
        // Admin bar colors
        $admin_bar_bg = $settings['admin_bar_bg'] ?? $settings['admin_bar_background'] ?? null;
        if ($admin_bar_bg) {
            $css .= "    --mas-adminbar-bg-color: {$admin_bar_bg};\n";
            $css .= "    --mas-adminbar-bg: {$admin_bar_bg};\n"; // Alias for JS
        }
        
        if (isset($settings['admin_bar_text_color'])) {
            $css .= "    --mas-adminbar-text-color: {$settings['admin_bar_text_color']};\n";
            $css .= "    --mas-adminbar-text: {$settings['admin_bar_text_color']};\n"; // Alias for JS
        }
        
        // Admin bar dimensions
        if (isset($settings['admin_bar_height'])) {
            $css .= "    --mas-adminbar-height: {$settings['admin_bar_height']}px;\n";
        }
    ...
    private function generateAdminBarCSS($settings) {
        ...
        // Check if floating is enabled - don't reset position if it is
        $isFloating = isset($settings['admin_bar_floating']) && $settings['admin_bar_floating'];
        ...
        if (isset($settings['admin_bar_gradient_enabled']) && $settings['admin_bar_gradient_enabled']) {
            ...
            $css .= "background: {$gradient} !important;";
             ...
             // Use CSS Variable for instant live preview
             $css .= "background: var(--mas-adminbar-bg, #23282d) !important;";
         }
        ...
        // Wysokość - use CSS Variable for instant live preview
        $height = $settings['admin_bar_height'] ?? 32;
        $css .= "height: var(--mas-adminbar-height, {$height}px) !important;";
        $css .= "min-height: var(--mas-adminbar-height, {$height}px) !important;";
        ...
        if (isset($settings['admin_bar_shadow']) && $settings['admin_bar_shadow']) {
         ...
         // Use CSS Variables for instant live preview
         $css .= "
         #wpadminbar .ab-item,
         #wpadminbar a.ab-item,
         #wpadminbar > #wp-toolbar span.ab-label,
         #wpadminbar > #wp-toolbar span.noticon {
             color: var(--mas-adminbar-text, #eee) !important;
         }
         ...
         #wpadminbar .ab-top-menu > li:hover > .ab-item {
             color: var(--mas-adminbar-text, #fff) !important;
        ...
        // Icon size
        if (isset($settings['admin_bar_icon_size'])) {
            $iconSize = $settings['admin_bar_icon_size'];
        ...
        if (isset($settings['admin_bar_spacing'])) {
            $spacing = $settings['admin_bar_spacing'];
            $css .= "#wpadminbar .ab-top-menu > li {
                         margin-left: {$spacing}px !important;
                     }
                     #wpadminbar .ab-top-menu > li:first-child {
    ...
    private function generateMenuCSS($settings) {
        ...
        // Use CSS Variables for instant live preview
        $css .= "#adminmenu {\n";
        $css .= "    background: var(--mas-menu-bg, #23282d) !important;\n";
        $css .= "}\n";
        
        $css .= "#adminmenu a {\n";
        $css .= "    color: var(--mas-menu-text, #eee) !important;\n";
        $css .= "}\n";
        
        $css .= "#adminmenu li.menu-top:hover > a {\n";
        $css .= "    background: var(--mas-menu-hover, #1d2327) !important;\n";
        $css .= "    color: var(--mas-menu-hover-text, #fff) !important;\n";
        $css .= "}\n";
        
        $css .= "#adminmenu li.current > a,\n";
        $css .= "#adminmenu li.wp-has-current-submenu > a {\n";
        $css .= "    background: var(--mas-menu-active, #0073aa) !important;\n";
        $css .= "    color: var(--mas-menu-active-text, #fff) !important;\n";
        $css .= "}\n";
        
        $css .= "#adminmenu .wp-submenu {\n";
        $css .= "    background: var(--mas-submenu-bg, #2c3338) !important;\n";
        $css .= "}\n";
        
        $css .= "#adminmenu .wp-submenu a {\n";
        $css .= "    color: var(--mas-submenu-text, #eee) !important;\n";
        $css .= "}\n";
        
        $css .= "#adminmenu .wp-submenu a:hover {\n";
        $css .= "    background: var(--mas-submenu-hover, #32373c) !important;\n";
        $css .= "    color: var(--mas-submenu-hover-text, #fff) !important;\n";
        $css .= "}\n";
    ...
    private function generateEffectsCSS($settings) {
        ...
        if (isset($settings['glassmorphism_effects']) && $settings['glassmorphism_effects']) {
            ...
            // TEMPORARY: Menu glassmorphism disabled
            // if (isset($settings['menu_glassmorphism']) && $settings['menu_glassmorphism']) {
            //     $css .= "#adminmenuwrap {\n";
        ...
        if (isset($settings['enable_shadows']) && $settings['enable_shadows']) {
            ...
            // TEMPORARY: Menu shadows disabled
            // if (isset($settings['menu_shadow']) && $settings['menu_shadow']) {
            //     $css .= "#adminmenuwrap {\n";
        ...
        if (isset($settings['enable_animations']) && $settings['enable_animations']) {
            ...
            // Base transitions for all interactive elements (menu excluded temporarily)
            $css .= ".postbox, .meta-box-sortables .postbox,\n";
            $css .= ".button, .button-primary, .button-secondary,\n";
            ...
            // TEMPORARY: Submenu slide animations disabled
